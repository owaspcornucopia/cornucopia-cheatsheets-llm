...
[Both implementations](#implementations) use an internal HTTP channel between the API service and the model service. This channel carries:
...
Ensure the messages sent to the model service are constructed server-side and never echo raw user input as a system message. In [both implementations](#implementations), the question is currently placed in a user-role message rather than a system-role message.
