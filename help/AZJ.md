...
The access control "list" (the allowed_tokens array) is hardcoded in [Python implementation](https://github.com/owaspcornucopia/llm-companion-scenario) source code. It is not accessible through the application's API or any database query. An attacker cannot:

- Query the token list through the `/api/fraud` endpoint
- Retrieve authorization configuration via SQL injection (the tokens are in [Python implementation](https://github.com/owaspcornucopia/llm-companion-scenario) memory, not in the database)
